...
function Carousel({ name, images, isLink }: CarouselProps) {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            setActiveIndex((prevActiveIndex) => (prevActiveIndex === images.length - 1 ? 0 : prevActiveIndex + 1));
        }, 3000);
        ...
        <div className="relative flex flex-col items-center gap-4 z-20">
            <div className="images">
                {images.map((img, index) => (
                    <div key={index} className="animate-pulse bg-slate-800">
                        <Image
                            src={img}
                            alt={name}
                            height={0}
                            ...
                            style={{ width: '100%', height: isLink ? '200px' : '400px' }}
